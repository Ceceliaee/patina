...
这些扩展包可以随 Patina Release 一起分发，但扩展内部 `manifest.json` 的 `version` 独立管理。只有对应浏览器系扩展自身发生可发布变化时，才更新该扩展版本号。
附件文件名中的 `vX.Y.Z` 必须来自对应扩展自己的 `manifest.json`，例如 Chromium 系扩展版本为 `0.1.0` 时，附件名为 `patina-chromium-extension-v0.1.0.zip`；Firefox 系扩展同理使用 `patina-firefox-extension-v0.1.0.xpi`。
Chromium 系 zip 内部必须包含同名扩展目录，例如 `patina-chromium-extension-v0.1.0/`，避免用户解压时把扩展运行文件散落到当前目录。
Firefox 系正式 Release 只发布经 Mozilla AMO `unlisted` 签名的 `.xpi`，不发布 Firefox zip 作为用户安装附件。未签名 zip 仅允许作为本地开发、临时调试或人工排查产物，不进入 GitHub Release 附件。
浏览器扩展的用户配置说明由 Patina 设置页承载；扩展项目 README 只保留在仓库中，不随 zip 或 xpi 附带。Chromium 系用户安装时不能直接通过“加载未打包”选择 zip 文件；应先解压 zip，再在浏览器扩展页选择解压后包含 `manifest.json` 的目录。Firefox 系用户应通过 Firefox 附加组件管理器从文件安装签名 `.xpi`。
...
- R2 不同步 `patina-chromium-extension-vX.Y.Z.zip` 或 `patina-firefox-extension-vX.Y.Z.xpi`；浏览器扩展包只作为 GitHub Release 附件分发
...
`write-release-notes`、`npm run tauri build -- --bundles nsis`、`npm run extension:chromium:package`、`npm run extension:firefox:sign` 与 `npm run release:prepare-assets`
...
`npm run extension:firefox:sign` 需要 Mozilla AMO API 凭据，工作流使用 `AMO_JWT_ISSUER` 与 `AMO_JWT_SECRET` secrets 注入为 `WEB_EXT_API_KEY` 与 `WEB_EXT_API_SECRET`。
...
8. 由工作流生成 release notes、构建安装包、打包 Chromium 扩展、签名 Firefox `unlisted` `.xpi` 扩展与 GitHub 版 `latest.json`。
9. 由工作流发布 GitHub Release，附件至少包含 Windows 安装包、`latest.json`、`patina-chromium-extension-vX.Y.Z.zip` 与 `patina-firefox-extension-vX.Y.Z.xpi`。
