...
- 简短、清晰、避免内部术语
- 优先说明用户能感知到的变化
- 中英文字段必须表达同一组发布事实；updater 按界面语言选择，俄语和西班牙语等没有独立字段的界面回退到英文
...
应用内更新提示默认使用对应版本节的 `App note:` 或 `App note en:`，而不是完整 release 正文；简体中文使用中文字段，其他界面语言优先使用英文，旧版本缺少英文时再回退到中文。
