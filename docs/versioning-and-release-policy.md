...
zip 内部必须包含同名扩展目录，例如 `patina-chromium-extension-v0.1.0/`，避免用户解压时把扩展运行文件散落到当前目录。
浏览器扩展的用户配置说明由 Patina 设置页承载；扩展项目 README 只保留在仓库中，不随 zip 附带。用户安装时不能直接通过“加载未打包”选择 zip 文件；应先解压 zip，再在浏览器扩展页选择解压后包含 `manifest.json` 的目录。
