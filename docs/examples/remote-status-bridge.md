...
- `访问令牌` 为空时，即使开关打开也不会连接。
- `本机标识` 由 Rust 启动时生成并保存，设置页只读展示。

修改 `接收地址`、`访问令牌`、`本机标识` 或开关后，旧连接会关闭。若新配置可用，Patina 会重新连接 Worker。
...
访问令牌由用户自行设置。Patina 只负责把它放进 `auth` 消息。
...
| 阶段 | 方向 | 消息体 | 说明 |
| --- | --- | --- | --- |
| 鉴权 | Patina -> Worker | `{"type":"auth","token":"访问令牌"}` | 连接建立后第一条消息 |
