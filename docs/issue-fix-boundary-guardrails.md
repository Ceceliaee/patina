...
- 按根 [`AGENTS.md`](../AGENTS.md#documentation-hygiene) 判断长期文档更新准入；契约不变的实现修复可以不改母文档；
...
前六类升级模式并核对对应长期 owner，不能把重新判断当作自动扩权；现有授权和规范不足以确定方案时才请求方向。后两类暂停受影响的动作并请求方向或权限；独立且已授权的工作继续。新视觉方向、不可逆数据操作等仍受其专门授权要求约束。
...
按 [`engineering-quality.md`](./engineering-quality.md#5-默认验证门槛) 区分文档、前端行为和 Rust/架构实现/发布风险；命令图由 `package.json` 拥有。无法运行的检查说明原因、残余风险和验证 owner。
...
- 与任务无关的用户改动未被覆盖；
- 风险匹配验证通过，未运行项有明确说明；
- 临时执行单中需要持久化的规则已更新真实 owner，其余执行记录按生命周期归档。
