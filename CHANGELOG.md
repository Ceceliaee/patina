...
- 应用会记住上次停留的主页面，普通启动和更新重启都会尽量回到原页面；后台停留较久后不再强制回到 Dashboard 或今天。
- 低耗后台的前端缓存释放、主窗口 WebView 释放和小组件空闲释放统一缩短到 3 分钟，追踪继续保留。
- 数据页减少常规进入时的可见 loading，应用趋势拆成更轻的渲染路径，搜索和热力图悬浮提示响应更稳。
- 历史时间线弹窗新增居中的日期切换，可在弹窗内前后切换日期，并同步外层历史页日期。
- 网页时间线以域名作为主行标题，页面标题放入详情并按同一时长口径汇总，减少标题变化造成的重复分组。
...
- 修复历史时间线弹窗在切换日期或应用 / 网页模式时高度可能跳动的问题。
- 修复设置页本机目录和设置快照刷新后状态可能滞留的问题。
