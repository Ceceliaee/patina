...
- 应用与网页的单项排除改为匿名统计，统一显示匿名时长；恢复追踪后保留匿名记录并重新显示既有历史。存在匿名网页规则且当前网页身份未知时，不保存浏览器标题，活动时间照常统计。
...
Release: 修复已删除网页卡片重新出现的问题，并改善删除后的刷新、提示与键盘操作。
App note: 已删除的普通网页卡片不再因残留设置重新出现，删除后的刷新与键盘操作更可靠。
App note en: Fix deleted website cards reappearing and improve refresh and keyboard navigation after deletion.

### Fixed

- 修复已删除历史的普通网页卡片因残留设置在刷新、重启或升级后重新出现的问题；保留排除和关联管理，再次访问后正常记录新活动。
- 修复网页关联主项没有历史时，删除操作可能误指向成员的问题，确保仅删除指定完整域名的历史。
- 改善网页删除后的反馈：刷新失败可重试，成功提示正确描述历史清理，卡片消失后恢复键盘焦点。

## [1.9.7] - 2026-09-17

Release: 新增应用与网页关联、托盘图标开关和 WebDAV 上传确认，改善图标更新、数据读取与恢复可靠性。
App note: 支持关联应用与网页、隐藏托盘图标，远程备份上传前可确认文件名，并修复图标、时间轴及数据读取问题。
App note en: Link apps and websites, hide the tray icon, confirm backup names, and get fixes for icons, timelines, and data loading.

### Added

- 分类页支持手动关联应用与网页，统一使用主项的名称、图标、分类和颜色展示统计；原始记录保留，解除后恢复独立展示，标题记录、排除与删除仍作用于原程序或域名。
- 设置中新增“显示托盘图标”开关。隐藏图标后仍可继续后台记录，并通过再次启动 Patina 打开主窗口。Refs [#78](https://github.com/Ceceliaee/patina/issues/78)

### Changed

- 手动上传 WebDAV 备份前可确认目标目录、修改文件名或取消；缺失的备份目录会按需创建，同名文件不会被覆盖。
- 网页分类采用与应用分类一致的紧凑布局和图标操作，保留颜色编辑、标题记录、排除及删除控制。

### Fixed

- 修复应用展示中的陈旧图标和错误图例：图标缓存满 30 天后在新会话中后台核验，等待或失败时保留旧图；历史时间轴按分类显示时使用分类名称与颜色。
- 修复数据读取和恢复失败后的状态不一致：今天、历史和数据页面保留可信内容并支持重试，旧请求不再覆盖新结果，备份恢复、工具操作及计划任务保留已成功保存的状态。
- 更新 Rustls 至 0.23.45，修复 TLS 握手加密层级校验漏洞 RUSTSEC-2026-0285。

### Removed

### Internal

- 将包管理器从 npm 迁移至 pnpm，构建与发布流程统一使用固定版本的 Node.js 和 pnpm。Refs [#56](https://github.com/Ceceliaee/patina/issues/56)
- 加强发布验证、真实桌面恢复测试及长期数据量下的资源预算检查。
...
Release: 修复应用与网页计时及详情统计，支持记录UWP应用，并改善短活动查看与快捷分类。
App note: 应用与网页计时更准确，详情统计更一致，支持UWP应用记录及查看不足一分钟的活动。
App note en: Improves app and website timing, detail totals, UWP app tracking, and viewing activities shorter than a minute.

### Added

### Changed

- 时间线与详情的最短时长筛选范围由1～10分钟扩展为0～10分钟；设为0可查看全部有效记录，两处分别保存选择。

### Fixed

- 修复暂停、锁屏、休眠恢复及采样失败后可能多记应用或网页时长的问题；网页记录随有效浏览器会话结束，备份恢复也保留原有计时边界。
- 修复网页总览与详情因重叠记录重复累计、将短暂空档计入时长而出现的差异；相同日期与范围内，未筛选详情与总览使用一致的有效时长。
- 修复短活动在时间线合并及时间轴分轨统计中遗漏的问题；保留现有分组与每分钟主要对象的概括，穿插活动仍计入各自时长，详情缩放时空档不计时。
- 修复应用快捷操作在活动记录的进程名大小写与已保存规则键不一致时，重命名会清除分类、修改分类又会恢复旧名称的问题；应用名称与分类现在会相互保留，保存成功后也不再误报“分类规则已更新”。
- 修复下拉菜单刷新选项时键盘高亮被重置的问题，使用方向键选择时保持当前高亮。
- 修复UWP应用（如 Microsoft Store、计算器）由 Windows 系统窗口宿主承载时无法记录的问题；现在会以真实应用身份保存活动，并显示对应的本地化名称和图标。Refs [#75](https://github.com/Ceceliaee/patina/issues/75)
