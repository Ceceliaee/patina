...
The current maintainer-curated execution order is published in the
[Patina Development Queue](https://github.com/users/Ceceliaee/projects/1),
which is also available from the
[repository Projects page](https://github.com/Ceceliaee/patina/projects).
...
1. Check the [Patina Development Queue](https://github.com/users/Ceceliaee/projects/1)
   for accepted, current, or overlapping work.
2. Search existing issues and pull requests for related work.
3. Read the relevant active project documents.
4. Identify the real owner of the behavior before choosing a file.
5. Decide whether the task is a focused change or a broader boundary change.
6. Keep unrelated cleanup out of the pull request.

The Development Queue is a maintainer-curated execution view, not a complete
list of feedback or an automatic assignment system. An item's position and
status communicate current planning; they do not change any GitHub Issue state
or grant permission to broaden its scope. Before starting non-trivial work on a
Project item, especially one marked `Next`, `In progress`, or `Blocked`,
coordinate with the maintainer to avoid duplicate or conflicting work.
...
- [ ] I checked the public Development Queue for current or overlapping work.
...
维护者筛选后的当前执行顺序发布在
[`Patina Development Queue`](https://github.com/users/Ceceliaee/projects/1)，
也可以从[仓库 Projects 页面](https://github.com/Ceceliaee/patina/projects)进入。
...
1. 检查 [`Patina Development Queue`](https://github.com/users/Ceceliaee/projects/1)，
   确认是否已有已接受、正在进行或范围重叠的工作。
2. 搜索是否已有相关 issue 和 Pull Request。
3. 阅读相关的有效项目文档。
4. 先识别行为的真实 owner，再选择文件。
5. 判断任务是聚焦的小改动，还是更广泛的边界调整。
6. 不要在 Pull Request 中混入无关清理。

Development Queue 是维护者筛选后的执行视图，不是完整反馈列表，也不是自动认领系统。
工作项的位置和状态用于表达当前计划，不会改变任何 GitHub Issue 状态，也不代表可以扩大
工作项范围。开始处理 Project item 前，尤其是标记为 `Next`、`In progress` 或 `Blocked`
的事项，应先与维护者协调，避免重复或相互冲突的工作。
...
- [ ] 我阅读了相关的有效项目文档。
- [ ] 我检查了公开 Development Queue，确认没有正在进行或范围重叠的工作。
- [ ] 我的分支从近期 `main` 创建。
